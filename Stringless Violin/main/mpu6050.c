...
static esp_err_t mpu6050_write_reg(uint8_t reg, uint8_t value) {
    ...
    return i2c_master_write_to_device(MPU6050_I2C_PORT, MPU6050_ADDR, write_buf, sizeof(write_buf), pdMS_TO_TICKS(1000));
}

static esp_err_t mpu6050_read_reg(uint8_t reg, uint8_t *data, uint8_t len) {
    return i2c_master_write_read_device(MPU6050_I2C_PORT, MPU6050_ADDR, &reg, 1, data, len, pdMS_TO_TICKS(1000));
}
...
void mpu6050_init(void) {
    ...
    i2c_driver_install(MPU6050_I2C_PORT, conf.mode, 0, 0, 0);
...
void mpu6050_read(MPU6050_Data *data) {
    if (!mpu6050_initialized || !data) return;

    uint8_t buffer[14];
    mpu6050_read_reg(MPU6050_REG_ACCEL_XOUT_H, buffer, 14);
    ...
    // Convert to float (scale factors for ±2g accel, ±250°/s gyro)
    ...
    data->temp = (temp_raw / 340.0f) + 36.53f;  // °C
}
